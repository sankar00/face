...
class App:
    ...
    def register_new_user(self, name: str = Form(...)):
        self.process_webcam()
...
@app.post("/logout")
def logout(name: str = Form(...)):
    return app_instance.logout(name)

@app.post("/register")
def register_new_user(name: str = Form(...)):
